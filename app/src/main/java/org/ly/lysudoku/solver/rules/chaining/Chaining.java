...
import org.ly.lysudoku.tools.LinkedSet;
...
//@SudokuMonster: Changes for variants
    ...
    static Potential.Cause getRegionCause(Grid.Region region) { //still in use by collectRuleParents where for regionchaining region is used for repaint and stings
        if (region instanceof Grid.Block && Settings.getInstance().isBlocks())
...
//@SudokuMonster: Variants changes
         else if (region instanceof Grid.DG && Settings.getInstance().isDG())
            return Potential.Cause.HiddenDG;
         else if (region instanceof Grid.Window && Settings.getInstance().isWindows())
            return Potential.Cause.HiddenWindow;
         else if (region instanceof Grid.diagonalMain && Settings.getInstance().isX())
            return Potential.Cause.HiddenMD;
         else if (region instanceof Grid.diagonalAnti && Settings.getInstance().isX())
            return Potential.Cause.HiddenAD;
         else if (region instanceof Grid.Girandola && Settings.getInstance().isGirandola())
            return Potential.Cause.HiddenGirandola;
         else if (region instanceof Grid.Asterisk && Settings.getInstance().isAsterisk())
            return Potential.Cause.HiddenAsterisk;
         else if (region instanceof Grid.CD && Settings.getInstance().isCD())
    ...
    private Set<Potential> getOffToOn(Grid grid, Potential p, Grid source,
        ...
        if (isXChainEnabled) {
			...
			for (int regionTypeIndex = (Settings.getInstance().isBlocks() ? 0 : 1); regionTypeIndex < (Settings.getInstance().isVLatin() ? 3 : 10); regionTypeIndex++) {
				...
				Grid.Region r = Grid.regions[regionTypeIndex][Grid.cellRegions[thisCellIndex][regionTypeIndex]];
				int otherPosition = -1;
