...
import org.ly.lysudoku.gui.SudokuBoardView.OnCellSelectedListener;
import org.ly.lysudoku.gui.SudokuBoardView.OnCellTappedListener;
...
public class IMControlPanel extends LinearLayout {
    ...
    private OnCellTappedListener mOnCellTapListener =(grid,cell)  -> {
        if (mActiveMethodIndex != -1 && mInputMethods != null) {
            mInputMethods.get(mActiveMethodIndex).onCellTapped(grid, cell);
        }
    };
    private OnCellSelectedListener mOnCellSelected = (grid,cell) -> {
        if (mActiveMethodIndex != -1 && mInputMethods != null) {
            mInputMethods.get(mActiveMethodIndex).onCellSelected(grid,cell);
        }
    };
